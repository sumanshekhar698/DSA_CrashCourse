package com.etechniketan.dsa.arrays.two_pointers;
    ...
    public static int trap(int[] height) {
        ...
        while (i < j) {//loop condition
            if (maxL <= maxR) {
                ...
                maxWater += Math.max(water, 0);//if water is -ve then we don't need to add it
...
//        => min(L,R =height[i]) for every i position where is L,R are the max height in L,R
        ...
        for (int i = height.length - 2; i >= 0; i--) {//enrich maxR
            if (height[i + 1] > max) {
                max = height[i + 1];
            }
            maxR[i] = max;
